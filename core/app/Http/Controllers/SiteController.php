...
class SiteController extends Controller
{
    ...
    public function agentshowSeat($id){
        ...
        return view($this->activeTemplate.'agent_book_ticket', compact('pageTitle','trip' , 'stoppages','busLayout', 'layout','mobile_code','countries'));
    }
    public function getTicketPrice(Request $request){
        ...
        $fleet_type_id_data = Trip::where('id', $request->trip_id)->get(['fleet_type_id']);
        $fleet_type_id = $fleet_type_id_data[0]->fleet_type_id;
        $totalseats_data = FleetType::where('id',$fleet_type_id)->get(['deck_seats']);
        ...
        if(sizeof($bookedTicket) == 0){
            $data['left_seats'] = $totalseats[0] - sizeof($bookedTicket);
        }else{
            $booked = 0;
            foreach($bookedTicket as $bt){
                $booked = $booked + $bt['ticket_count'];
            }
            $data['left_seats'] = $totalseats[0] - $booked;
        }
        return response()->json($data);
    }
    ...
    public function agentticketSearch(Request $request)
    {
        if($request->pickup && $request->destination && $request->pickup == $request->destination){
            $notify[] = ['error', 'Please select pickup point and destination point properly'];
            return redirect()->back()->withNotify($notify);
        ...
        return view($this->activeTemplate.'agent_ticket', compact('pageTitle' ,'fleetType', 'trips','routes', 'schedules', 'emptyMessage', 'layout'));
    }
